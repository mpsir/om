...
<div v-if="typeof Value == 'function'">
    ...
    "j_array_basic": {
        ...
        <div style="color:black" class="text-md text-700">[</div>
        <div>
            <q-btn @click="show_new_block = !show_new_block" flat class="text-800 m-0 p-0"> + </q-btn>
            <div v-if="show_new_block" class="border-dashed border-300 p-0 m-0">
                ...
                <q-btn @click="add_new_fun()" class="p-0 m-2 text-700" flat>Submit</q-btn>
            </div>
        </div>
        <div v-for="(a, a_no) in Value" class="my-0 px-2">
            <div>
                    ...
                    <template v-slot:control>
                        <div>
                            <button no-caps class="toggle-handle m-0 text-800 p-0" style="background-color:transparent; border:none; cursor:pointer">
                                ...
                                <span class="text-400 m-0 p-0">
                                    <button class="p-0 px-2 text-1000 m-0" style="background-color:transparent; border:none; cursor:pointer">
                                    {{ Value[a_no].toString().length > 25 ? Value[a_no].toString().slice(0, 25) + " ..." : Value[a_no] }}
...
<div v-if="show_wrapper" style="color:black" class="text-md text-700">[</div>
...
<toggle-content :show_inner_p="false">
    <template v-slot:control>
        <div>
            <button no-caps class="toggle-handle m-0 text-800 p-0" style="background-color:transparent; border:none; cursor:pointer">
                <span class="m-0 p-0 text-1000" style="color:black"> {{a[0]}} :  </span>
                <span class="text-400 m-0 p-0">
                    <button class="p-0 px-2 text-1000 m-0" style="background-color:transparent; border:none; cursor:pointer">
                    {{ Value[a_no][1].toString().length > 25 ? Value[a_no][1].toString().slice(0, 25) + " ..." : Value[a_no][1] }}
                    ...
                    <div class="p-0 m-0 mx-3">
                       <input style="border:none" @blur="Value[a_no][0] = $event.target.value; update_parent()" 
                        :Value="Value[a_no][0]" />  
                    </div>
                    <div class="p-0 m-0">
                        <select style="background-color:transparent; border:none" v-model="new_data_selected" class="p-1 mx-2">
                        ...
                        <button @click="Value[a_no][1] = change_type(); update_parent()" class="mx-2" style="background-color:transparent; border:none">Change</button>
                    </div>
                    </q-popup-proxy>
                  </button>
                </span>
